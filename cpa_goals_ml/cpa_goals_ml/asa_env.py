...
class ASAEnv(gym.Env):
    ...
    def __init__(self, keywords, client, probabilities):
    ...
    def calculate_reward(self,
        ...
        # Get moving average
        cvr_moving_avg = sum(self.hist["avg_cvr"][-self.window_size:]) / self.window_size
        ttr_moving_avg = sum(self.hist["avg_ttr"][-self.window_size:]) / self.window_size
    ...
    def reset(self):
        ...
        self.client.max_cpt_bid = self.client.ad_group.keyword_bids[self.keyword_name]
    ...
    def generate_bidders(self):
        ...
        for _ in range(nr_bidders):
            ...
            cpa_goal = random.uniform(0, avg_cpa * 1.2)
    ...
    def enter_acution(self):
        ...
        client_cpa_goal = self.client.cpa_goal
        #print("bid to beat", bid_to_beat)
        if client_cpa_goal >= bid_to_beat:
            return True
    ...
    def perform_auction(self):
        ...
        lower_limit = self.client.avg_cpa * 1
        upper_limit = self.client.avg_cpa * 1.5

        if self.client.cpa_goal == 0:
            enter_auction = True

        elif lower_limit <= self.client.cpa_goal <= upper_limit: #self.client.cpa_goal >= self.client.avg_cpa:
        ...
        if enter_auction:
            ...
            if client_score >= highest_score and self.client.can_afford(bid_to_beat):
                ...
                self.client.bid_winner(bid_to_beat)
            else:
                pass#print("lost auction")
        else:
